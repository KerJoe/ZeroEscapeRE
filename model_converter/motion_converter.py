...
class Motion:
    ...
    class Dynamic:
        ...
        def __init__(self, data, offset):
            ...
            for _ in range(animation_amount - 1):
                self.extras += [ Motion.DynamicExtra(data, offset) ]
    ...
    def __init__(self, filepath):
        ...
        for _ in range(dynamic_amount):
            self.dynamics += [ Motion.Dynamic(data, offset) ]


if __name__ == "__main__":
    filepath = Path("/home/misha/Documents/Projects/ZeroEscapeRE/model_converter/extracted_models/scenes/chara/phi/e02.motion")
    if len(sys.argv) >= 2:
        filepath = Path(sys.argv[1]).absolute()
    filename = Path(filepath).stem

    motion = Motion(filepath)
    ...
    for dynamic in motion.dynamics:
